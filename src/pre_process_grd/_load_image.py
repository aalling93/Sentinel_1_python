...
from ._get_functions import get_index_v2, get_coordinate
...
def s1_load(path, polarisation='all', location=None, size=None):
    ...
    annotation_temp = [load_data._load_annotation(os.path.join(path, 'annotation', file)) for file in xml_files]
    ...
    calibration_temp = [load_data._load_calibration(os.path.join(path_cal, file)) for file in cal_files]
