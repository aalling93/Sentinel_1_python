        ...
        if len(sys.argv) > 0:
            ...
            for arg in sys.argv[1:]:
                if arg == "--insecure":
                    ...
                    print(
                        " > Command line argument '{0}' makes no sense, ignoring.".format(
                            arg
                        )
                    )
...
#
#       python ./download-all-2022-02-16_10-55-15.py
#
#    Download all files in a Metalink/CSV: (downloaded from ASF Vertex)
#
#       python ./download-all-2022-02-16_10-55-15.py /path/to/downloads.metalink localmetalink.metalink localcsv.csv
#
...
class xml_bulk_downloader:
    ...
    def __init__(self, gdf):
        ...
        for i in range(len(self.gdf)):
            name = self.gdf.iloc[i].dwl_link.split("/")
            download_files.append(
                f"https://datapool.asf.alaska.edu/METADATA_{name[3]}_HD/S{name[-2][2]}/{name[-2]}.iso.xml"
            )

        # self.files = [ "https://datapool.asf.alaska.edu/SLC/SA/S1A_IW_SLC__1SDV_20190708T142448_20190708T142515_028027_032A4E_E948.zip" ]
        self.files = download_files
